...
| PinballSpring | `(1870, 640)` のピンクの反射板。真下／斜め下Dashで突入し反射方向を比較 |
...
`pinball_spring.tscn` は独立したピンクの反射板です。ローカル上向きが表面法線で、rotationに追従します。接触直前の速度を反射し、入射角と接線成分を反映します。Dash突入時もDash速度で反射してからDashを終了し、Springと同じ全能力回復を行います。
